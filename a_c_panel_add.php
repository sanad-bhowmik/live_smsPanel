...
if (isset($_SESSION['_u_li_'])){
	...
	$user_type_code_array = array('2','3');
			if(isset($_REQUEST['create_user_submit'])){
				...
				$user_valid_date=$_POST['validdate'];
				$chk_crdt=chk_sms_credit($user_allocated_credit,$login_user_name);
					if(in_array($user_type_code,$user_type_code_array) && ($chk_crdt==1)){
						
								if(add_new_user($new_user_name,$user_pass,$user_type,$user_allocated_credit,$user_valid_date,$_SESSION['_u_li_'])==1){
			...
			if(isset($_REQUEST['send_user_login_name'])){
...
$user_info['user_login_pass']="";
$user_info['user_valid_date']="";
$user_info['user_allocated_sms_credit']="";
				}
				
				
			if(isset($_REQUEST['edit_user_acc_submit_add'])){
				$prev_user_name=$_POST['prev_user_name'];
				$new_name=$_POST['new_user_name'];
				$new_pass=$_POST['new_pass'];
				$new_credit=$_POST['new_credit'];
				$newvaliddate=$_POST['user_valid_new_date'];
				$chk_crdt=chk_sms_credit($new_credit,$login_user_name);
					if(($user_type_code==3) && ($chk_crdt==1)){
						
						if(user_info_edit_add($prev_user_name,$new_name,$new_pass,$new_credit,$newvaliddate)==1){
						...
						if(user_info_edit_add_admin($prev_user_name,$new_name,$new_pass,$new_credit,$newvaliddate)==1){
			...
			if(isset($_REQUEST['edit_user_acc_submit_minus'])){
				$prev_user_name=$_POST['prev_user_name'];
				$new_name=$_POST['new_user_name'];
				$new_pass=$_POST['new_pass'];
				$new_credit=$_POST['new_credit'];
				$newvaliddate=$_POST['user_valid_new_date'];
					if($user_type_code==3){
						...
						if(user_info_edit_minus($prev_user_name,$new_name,$new_pass,$new_credit,$newvaliddate)==1){
						...
						if(user_info_edit_minus_admin($prev_user_name,$new_name,$new_pass,$new_credit,$newvaliddate)==1){
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="add_user">
                            ...
                            <table width="100%" border="0" cellspacing="0" cellpadding="5" align="center">
							  ...
							  <tr>
                                <td>Valid Date &nbsp;:</td>
                                <td>&nbsp;  <input name="validdate" id="validdate" onclick="showCalendarControl(this);" onfocus="this.value=''" type="text" /><br /></td>
                              </tr>
                              <tr>
                                <td></td>
                            	...
                            	<form name="edit_user_acc" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
                                	<input type="hidden" name="prev_user_name" value="<?php echo $user_info['user_login_name'];?>"/>
                                		<table width="100%" align="center" border="0" cellspacing="0" cellpadding="5">
												  ...
												  <tr>
													<td>Valid Date &nbsp;:</td>
													<td>&nbsp;  <input name="user_valid_new_date" id="user_valid_new_date" onclick="showCalendarControl(this);" onfocus="this.value=''" value="<?php echo $user_info['user_valid_date'];?>" type="text" /><br /></td>
												  </tr>
                                                  <tr>
                                                    <td>Allocate Credit&nbsp;:</td>
